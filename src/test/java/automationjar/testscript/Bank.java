...
public class Bank extends BaseClass {
	 ...
	 * */
	@Test
	public void takeFullPageSnapShot() throws Exception {
		...
		Thread.sleep(20000);
		// change window size
		getDriver().manage().window().setSize(new Dimension(516, 500));
		// set position
		getDriver().manage().window().setPosition(new Point(110, 5));
		Thread.sleep(20000);
		takeSnapShot(getDriver(), "mainPage3.png");
		Dimension d = getDriver().manage().window().getSize();
		assertTrue("ScrennShots has been takes successfully", d.height == 500 && d.width==516);
	...
	public void searchIncorrectStr() throws Exception {
		...
		assertEquals("https://english.leumi.co.il/SearchResults/", getDriver().getCurrentUrl());
		Thread.sleep(3000);
		takeSnapShot(getDriver(), "incorrectSearch.png");

		// check if massage appears 
		boolean IsResAsExpected = getDriver().findElement(By.id("GSAsearchText")).
								  getText().contains("did not match any document");
	 ...
	 * */
	@Test
	public void switchLoginOptionsCombobox() throws InterruptedException {
		//combobox object
	...
	public void logIn() throws Exception {
		...
		Thread.sleep(5000);
		takeSnapShot(getDriver(), "login.png");
